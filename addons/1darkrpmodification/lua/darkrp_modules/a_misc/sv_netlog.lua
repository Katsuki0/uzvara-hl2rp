...
-- hook.Add("OnGamemodeLoaded", "creatingFile", function ()
--     if not file.Exists( "net_log.txt", "DATA" ) then
--         file.Write("net_log.txt", "")
--         print ("[NEG LOG] File created. Waiting for writes.")
--     else
--         print ("[NET LOG] File already created!")
--     end
-- end)
...
--             file.Append( "net_log.txt", string.format("Net spam attempted on Net Message: %s Client: %s (STEAMID: %s) (IP: %s) \n", name, plyNick, plySteamid, plyIP))
--         end
...
--         file.Append( "net_log.txt", string.format("No receiving function for '%s' (net msg #%d) Client: %s (STEAMID: %s) (IP: %s) \n", name, i, plyNick, plySteamid, plyIP))
--         return
--     end
...
--     file.Append( "net_log.txt", string.format(curString, name, i, len/8/1024, len/8, plyNick, plySteamid, plyIP))

--     local status, error = pcall( function() func(len, client) end )
    
--     if not status then
--         ServerLog(string.format("Error during net message (%s). Reasoning: %s \n", name, error))
--         file.Append( "net_log.txt", string.format("Error during net message (%s). Reasoning: %s \n", name, error))
--     end
