...
local keypad_crack_time = CreateConVar("keypad_crack_time", "10", {FCVAR_ARCHIVE}, "Seconds for keypad cracker to crack keypad")
...
function SWEP:PrimaryAttack()
    ...
    if IsValid(ent) and tr.HitPos:Distance(self.Owner:GetShootPos()) <= 50 and (ent:GetClass() == "nn_polya") and (ent:GetMode() != 1) then
...
function SWEP:Succeed()
    ...
    elseif SERVER and ent:GetClass() == "nn_polya" and ent:GetMode() != 1 then ent:Xyi(self.Owner)
    end
...
function SWEP:Think()
    ...
    if self.IsCracking and IsValid(self.Owner) then
        ...
        if not IsValid(tr.Entity) or tr.HitPos:Distance(self.Owner:GetShootPos()) > 100 or (tr.Entity:GetClass() != "nn_polya") then
            self:Fail()
...
if(CLIENT) then
    ...
    net.Receive("KeypadCracker_Hold", function()
        ...
        if IsValid(ent) and ent:IsWeapon() and ent:GetClass():lower() == "weaponchecker" and not game.SinglePlayer() and ent.SetWeaponHoldType then
            if not state then
