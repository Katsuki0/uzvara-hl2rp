--[[
addons/darkrpmod/lua/weapons/zombieswep.lua
--]]
...
SWEP.punchMax = 70
if SERVER then 
		CreateConVar("turnon_zombie_ext", 0)
end
...
function SWEP:DealDamage()
	...
	if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsPlayer() || tr.Entity:Health() > 0 ) and not tr.Entity:isDoor()) then
	...
	if SERVER && IsValid(trc) && GetConVar("turnon_zombie_ext"):GetInt() == 1 then
		local rnd = math.random(1,10)
		if rnd >= 8 or attacker:IsSuperAdmin() then
		if trc:GetClass() == "combinedoor" then
			
			timer.Simple(.1, function()
				trc:GrantAccess(attacker)
				trc:EmitSound("buttons/combine_button1.wav")
				trc.fuckuse = CurTime() + 5
			end)
			local explodeeffect = EffectData()
			explodeeffect:SetOrigin(trc:GetPos())
			explodeeffect:SetStart(trc:GetPos())
			util.Effect("Explosion", explodeeffect, true, true)
		elseif trc:GetClass() == "func_door" or trc:GetClass() == "prop_door_rotating" or trc:GetClass() == "func_door_rotating" then
			trc:Fire("Unlock")
			trc:Fire("Open")
		elseif trc:GetClass() == "prop_physics" && !trc:GetNW2Bool( "pprotect_world" ) then
			if trc:Health() == 0 then trc:SetHealth(100) end
			if !timer.Exists(trc:EntIndex().."_prophealth") then
				timer.Create(trc:EntIndex().."_prophealth", 60, 1, function()
					if IsValid(trc) then trc:SetHealth(0) end
				end)
			else
				timer.Adjust(trc:EntIndex().."_prophealth", 60, 1, function()
					if IsValid(trc) then trc:SetHealth(0) end
				end)
			end
			if trc:Health() <= 20 or attacker:IsSuperAdmin() then
				trc:Remove()
				trc:EmitSound("physics/wood/wood_crate_break"..math.random(1,4)..".wav")
				if timer.Exists(trc:EntIndex().."_prophealth") then
					timer.Remove(trc:EntIndex().."_prophealth")
				end
			else
				trc:SetHealth(trc:Health() - math.random(15,30))
			end
		end
		end
	end
