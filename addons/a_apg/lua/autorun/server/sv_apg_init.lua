...
concommand.Add("apg_set", function( ply, cmd, args, argStr )
	...
	if args[1] == "module" then
		...
		if _module != nil then
			...
			APG.notification( "[APG] Этот модуль не существует", ply)
		end
	...
	elseif args[1] == "help" then
		...
		if cfg then
			...
			APG.notification( "[APG] Справка: такого параметра нету", ply)
		end
	else
		APG.notification( ply, "Ошибка: неизвестная настройка")
	end
end)
