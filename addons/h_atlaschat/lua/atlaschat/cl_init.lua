...
hook.Add("DrawOverlay", "atlaschat.DrawCopiedNotify", function()
	for i = 1, #copiedNotify do
		...
		if (data) then
			draw.SimpleText("Текст скопирован!", "atlaschat.theme.text.shadow", data.x, data.y, color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
			draw.SimpleText("Текст скопирован!", "atlaschat.theme.text", data.x +1, data.y +1, color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
			draw.SimpleText("Текст скопирован!", "atlaschat.theme.text", data.x, data.y, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
...
hook.Add("player", "atlaschat", function(data)
	local filtered = atlaschat.filterJoinDisconnect:GetBool()

	if (!filtered) then
		chat.AddText(color_white, "", color_red" (" .. data.networkid	.. "", color_white, "" .. data.reason)
	end
end)
...
net.Receive("atlaschat.plcnt", function(bits)
	...
	if (!filtered) then
		chat.AddText(color_white, "Игрок ", color_limegreen, name, color_grey, " (" .. steamID .. ") ", color_white, "зашёл на сервер.")
	end
end)
...
net.Receive("atlaschat.sinvpm", function(bits)
	...
	chat.AddText(color_limegreen, player:Nick(), color_white, " Пригласил вас в приватный чат. Нажмите сюда чтоб ", accept)
end)
