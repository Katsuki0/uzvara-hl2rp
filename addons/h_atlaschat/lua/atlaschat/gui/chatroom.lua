...
function panel:Init()
	...
	self:SetCursor("hand")
	self:SetToolTip("Щелкните правой кнопкой мыши, чтобы покинуть чат")
end
...
function panel:RemovePlayer(player, left)
	...
	if (player == LocalPlayer()) then
		...
		if (!left) then
			...
			chat.AddText(color_red, ":exclamation: Вас исключили из комнаты!")
		end
	...
	-- Some other player was kicked or left the chatroom
		...
		if (left) then
			atlaschat.theme.Call("ParseText", list, ":information: " .. nick .. " Вышел из комнаты.")
		else
			atlaschat.theme.Call("ParseText", list, color_red, ":exclamation: " .. nick .. " Был исключён из комнаты.")
		end
...
function panel:OnMousePressed(code)
	...
	elseif (code == MOUSE_RIGHT and !self.name) then
		local menu = DermaMenu()
			local option = menu:AddOption("Выйти из беседы", function()
