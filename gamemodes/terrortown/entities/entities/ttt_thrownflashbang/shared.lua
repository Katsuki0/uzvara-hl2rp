...
if (CLIENT) then
	...
	function SimulateFlash_REALCS_NOT_ANYTHINGELSE() 
		if LocalPlayer():GetNetworkedFloat("RCS_flashed_time") > CurTime() then
			...
			local e = pl:GetNetworkedFloat("RCS_flashed_time");
			local s = pl:GetNetworkedFloat("RCS_flashed_time_start");
	...
	local function SimulateBlur_REALCS_NOT_ANYTHINGELSE()
		local pl = LocalPlayer();
		local e = pl:GetNetworkedFloat("RCS_flashed_time") + EFFECT_DELAY; 
		local s = pl:GetNetworkedFloat("RCS_flashed_time_start");
