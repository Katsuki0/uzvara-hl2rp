...
if CLIENT then
  ...
  SWEP.PrintName = "Маскировка под игрока"
  SWEP.Slot = 7
  ...
  SWEP.EquipMenuData = {
    type = "Weapon",
    desc = "Ворует личность чужого игрока."
  };
    ...
    local function AdvDisguiserInit()
     ...
     if not IsValid(ent) or not ent:IsPlayer() or ent.NoTarget then return end
      ...
      elseif (client:IsTraitor() or (client.IsEvil and client:IsEvil())) and (ent:IsTraitor() or (ent.IsEvil and ent:IsEvil())) or client:IsSpec() then
        text = ent:Nick() .. " (Замаскирован под " .. ent:GetNWString("AdvDisguiseName") .. ")"
        color = COLOR_RED
      else
...
if CLIENT then
  ...
  net.Receive("TTTAdvDisguiseSuccess",function()
    ...
    chat.AddText("Маскировка: ", COLOR_WHITE, "Личность " .. printname .. " успешно скопирована")
    chat.PlaySound()
  end)
  net.Receive("TTTAdvDisguiseIdentity",function()
    chat.AddText("Маскировка: ", COLOR_WHITE, "Вам нужно сначала скопировать личность!")
    chat.PlaySound()
  end)
end
