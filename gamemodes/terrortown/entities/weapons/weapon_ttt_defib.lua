...
if CLIENT then
  ...
  function SWEP:DrawHUD()
    ...
    if state == STATE_PROGRESS then
      ...
      progressText = self:GetStateText() or "ОЖИВЛЯЕМ"
...
function SWEP:PrimaryAttack()
  ...
  if IsValid(tr.Entity) and tr.Entity:GetClass() == "prop_ragdoll" then
    if not tr.Entity.uqid then
      self:FireError("НЕУДАЧА - ОБЬЕКТ БЫЛ ЗАСТРЕЛЕН В ГОЛОВУ")
      return
    end
    ...
    self:FireError("НЕУДАЧА - ОБЬЕКТ НЕВОЗМОЖНО ОЖИВИТЬ")
  end
end
...
function SWEP:BeginDefib(ply, ragdoll)
  ...
  if not spawnPos then
    self:FireError("НЕУДАЧА - МАЛО МЕСТА")
    return
  end

  self:SetStateText("ОЖИВЛЯЕМ - "..string.upper(ply:Name()))
...
function SWEP:Think()
  ...
  if self:GetDefibState() == STATE_PROGRESS then
    ...
    if tr.Entity ~= self.TargetRagdoll then
      self:FireError("НЕУДАЧА - ЦЕЛЬ ПОТЕРЯНА")
      return
    end
...
function SWEP:HandleRespawn()
  ...
  if isTable(PowerRounds.CurrentPR) then
    ply:SetHealth(1)
  end
