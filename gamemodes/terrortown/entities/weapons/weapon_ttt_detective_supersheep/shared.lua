...
SWEP.PrintName			= "Разведовательная овечка"			  
SWEP.Author				= "TheBroomer"  
SWEP.Instructions			= "Запускает овечку для того что-бы отследить предателей!"
SWEP.Slot				= 1
SWEP.SlotPos			= 2
...
SWEP.EquipMenuData = {
	type = "item_weapon",
	desc = [[
	Запускает овечку для отслеживания предателей!
	ЛКМ: Пометить игрока
	ПКМ: Вернуть овечку
	]]
}
...
if not TTT2 then
	function SWEP:DrawHUD()
		if IsValid(self.Ent_supersheep) && self.SheepStartTime >= 0 then
			...
			ShadowedText("ЛКМ - начать следить за игроком", "HealthAmmo", ScrW() / 2, y + 10 + 12, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
