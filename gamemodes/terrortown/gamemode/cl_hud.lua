...
local function SpecHUDPaint(client)
   ...
   -- draw.RoundedBox(0, 0, 0, width, height, Color(60, 45, 132, 255))
-- 
   -- draw.RoundedBox(0, 0, 0, width, 30, Color(96, 96, 96, 255))
   if GetConVar("ttt_spectator_mode"):GetBool() == true then
      draw.SimpleTextOutlined("Для быстрого выхода из режима Наблюдателя нажмите F9", "HealthAmmo", ScrW()*0.25, 45, Color( 255, 255, 255, 255 ), 0, 0, 1, Color(0,0,0,255))
   end

   -- if LocalPlayer():GetNWInt("TraitorAbuse") != 0 and os.time >= LocalPlayer():GetNWInt("TraitorAbuse") then
   --    draw.SimpleTextOutlined("У вас есть возможность стать трейтором. Нажмите F4", "HealthAmmo", ScrW()*0.25, 45, Color( 255, 255, 255, 255 ), 0, 0, 1, Color(0,0,0,255))
   -- end
...
local function InfoPaint(client)
   ...
   if LocalPlayer():GetNWInt("TraitorAbuse") != 0 then
      if tostring(os.time()) >= tostring(LocalPlayer():GetNWInt("TraitorAbuse")) then
         if GetRoundState() == ROUND_PREP then
            draw.SimpleTextOutlined("У вас есть возможность стать трейтором. Нажмите F4", "HealthAmmo", ScrW()*0.3, 45, Color( 255, 255, 255, 255 ), 0, 0, 1, Color(0,0,0,255))
         end
      end
   end
...
hook.Add("PlayerButtonUp", "ExitSpectatorMode", function (ply, button)
   if ply.NextClick == nil or ply.NextClick <= CurTime() then
      if button == 100 then
      ...
      if button == 95 then
         ply:ConCommand("ttt_make_me_traitor_please")
      end

      ply.NextClick = CurTime() + 0.5
   end
end)