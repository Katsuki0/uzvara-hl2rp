...
-- function CanHear(l)
--     if mute_all then return end
--     if not IsValid(l) then return end
--     tttHear[l] = tttHear[l] or {}
--     if l:IsBot() then return end

--     for _, t in ipairs (player.GetHumans()) do
--         tttHear[l][t] = t:IsSpec() and l:IsSpec()
--         or not t:IsSpec() and l:IsSpec()
--         or not t:IsSpec() and not l:IsSpec()
--         or t:IsActiveTraitor() and l:IsActiveTraitor() and t.traitor_gvoice


--         // Я пока не знаю способа улучшить это. Думаю...
--         if l.mute_team == MUTE_ALL or l.mute_team == t:Team() then
--              tttHear[l][t] = false
--         end
--     end 
-- end

-- hook.Add("PlayerInitialSpawn", "tttCanHearVoice", function(ply)
--     CanHear(ply)
--     if ply:IsBot() then return end
--     timer.Create(ply:UserID() .. "ttCanHear", 0.5, 0, function () CanHear(ply) end)
-- end)

-- hook.Add("PlayerDisconnected", "tttDestroyTable", function(ply)
--     tttHear[ply] = nil
--     if ply:IsBot() then return end
--     for _, v in ipairs(player.GetHumans()) do
--         if not tttHear[v] then continue end
--         tttHear[v][ply] = nil
--     end
--     timer.Remove(ply:UserID() .. "ttCanHear")
-- end)

-- function GM:PlayerCanHearPlayersVoice(listener, talker)
--     -- if not deadv and not talker:Alive() then return false end

--     return tttHear[listener][talker], false
-- end
