...
function KARMA.ApplyKarma(ply)
   ...
   -- print (ply:GetBaseKarma())
   if tonumber(ply:GetBaseKarma()) < 1000 then
...
hook.Add("OnGamemodeLoaded", "KarmaDB", function ()
   if sql.TableExists("ttt_players_karma") then
      print ("[KarmaMode] DB already created")
   else
      sql.Query("CREATE TABLE ttt_players_karma( S64, karma)")
      print ("[KarmaMode] DB was created!")
   end
end)


-- hook.Add("PlayerInitialSpawn","KarmaDB.PlayerFirstSpawn",function (ply)
--    if ply:IsBot() then return end
--    local query = sql.Query("SELECT karma FROM ttt_players_karma WHERE S64 = '"..ply:SteamID64().."'")
--    if (query) then
--       timer.Simple(1, function ()
--          ply:SetBaseKarma(tonumber(math.Round(query[1].karma)))
--          ply:SetLiveKarma(tonumber(math.Round(query[1].karma)))
--          print ("[Karma] Restored "..query[1].karma.." for player "..ply:Nick().."(select)")
--       end)
--    end
-- end)


-- hook.Add("PlayerDisconnected", "karmaDB.PlayerLeaveServer", function (ply)
--    if ply:IsBot() then return end
--    local query = sql.Query("SELECT karma FROM ttt_players_karma WHERE S64 = '"..ply:SteamID64().."'")
--    if (query) then
--       sql.Query( "UPDATE ttt_players_karma SET karma = '"..tonumber(math.Round(ply:GetBaseKarma())).."' WHERE S64 = '"..ply:SteamID64().."'" )
--       print ("[Karma] Saved "..tonumber(math.Round(ply:GetBaseKarma())).." for player "..ply:Nick().."(update)")
--    else
--       sql.Query( "INSERT INTO ttt_players_karma( S64, karma ) VALUES( '"..ply:SteamID64().."', '"..tonumber(math.Round(ply:GetBaseKarma())).."' )" )
--       print ("[Karma] Saved "..tonumber(math.Round(ply:GetBaseKarma())).." for player "..ply:Nick().." (insert)")
--    end
-- end)

-- hook.Add("ShutDown", "karmaDB.SavePlayersCHangeLevelOrShutDown", function ()
-- 	for k,v in pairs (player.GetAll()) do
-- 		local query = sql.Query("SELECT karma FROM ttt_players_karma WHERE S64 = '"..v:SteamID64().."'")
--    		if (query) then
--      		sql.Query( "UPDATE ttt_players_karma SET karma = '"..tonumber(math.Round(ply:GetBaseKarma())).."' WHERE S64 = '"..v:SteamID64().."'" )
--       		print ("[Karma] Saved "..tonumber(math.Round(ply:GetBaseKarma())).." for player "..v:Nick().."(update)")
--    		else
--       		sql.Query( "INSERT INTO ttt_players_karma( S64, karma ) VALUES( '"..v:SteamID64().."', '"..tonumber(math.Round(ply:GetBaseKarma())).."' )" )
--       		print ("[Karma] Saved "..tonumber(math.Round(ply:GetBaseKarma())).." for player "..v:Nick().." (insert)")
--       	end
--    end
-- end)	

-- halo.Add("table entities", number blurX = 2, number blurY = 2, number passes = 1, boolean additive = true, boolean ignoreZ = false)