...
function plymeta:ShouldSpawn()
   ...
   -- if self:IsBanneD() then return false end

   -- if self:IsBanned() then
   --    self:ChatPrint("Вы забанены и не можете играть!")
   --    self:ConCommand("ttt_spectator_mode 1")
   --    return false
   -- end
...
function plymeta:SpawnForRound(dead_only)
   -- if self:IsBanned() then return self:ChatPrint("Вы забанены") self:RunConsoleCommand("ttt_spectator_mode", "1") end
