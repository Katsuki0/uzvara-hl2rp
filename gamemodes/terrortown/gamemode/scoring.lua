...
// My shitty code.
-- hook.Add("OnGamemodeLoaded", "ScoreDB", function ()
--    if sql.TableExists("ttt_all_score") then
--       print ("[ScoreMode] DB already created")
--    else
--       sql.Query("CREATE TABLE ttt_all_score( S64, score)")
--       print ("[ScoreMode] DB was created!")
--    end
...
-- hook.Add("PlayerInitialSpawn", "SendFirstTop10", function (ply)
--    net.WriteTable(top_10)
--    net.Send(ply)
-- end)



-- hook.Add("TTTEndRound", "SaveScoreToDB", function ()
--    // Не знаю на сколько этот метод жетский. Но попробуем первое время.
--    for k,v in pairs (player.GetAll()) do
--       local query = sql.Query("SELECT score FROM ttt_all_score WHERE S64 = '"..v:SteamID64().."'")
--       if query then
--          sql.Query( "UPDATE ttt_all_score SET score = score + '"..v:Frags().."' WHERE S64 = '"..v:SteamID64().."'" )
--       else
--          sql.Query( "INSERT INTO ttt_all_score( S64, score ) VALUES( '"..v:SteamID64().."', '"..v:Frags().."' )" )
--       end
--    end
-- end)

-- -- hook.Add("TTTEndRound", "SendTop10ToPlayers", function ()
-- 
-- end)
