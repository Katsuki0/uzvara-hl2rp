...
function PANEL:Init()
   ...
   self.rank_image = vgui.Create("DImage", self)
   self.rank_image:SetPos(29,5)
   self.rank_image:SetSize(16,16)
   self.rank_image:SetMouseInputEnabled(false)

   self.nick = vgui.Create("DLabel", self)
   -- self.nick:SetPos(200, 4)
   ...
   self.ranks_table =serverguard.ranks:GetStored()
end
...
function PANEL:UpdatePlayerData()
   ...
   for k,v in pairs (self.ranks_table) do
      if ply:GetUserGroup() == k then
         self.rank_image:SetImage(v.texture)
      end
   end
   -- self.rank_image:SetImage(self.r)
...
function PANEL:PerformLayout()
   ...
   self.nick:SetPos(SB_ROW_HEIGHT + 25, (SB_ROW_HEIGHT - self.nick:GetTall()) / 2)
...
function PANEL:DoClick(x, y)
   ...
   -- print (self.nick)
   -- print (self.Player)
   -- self:OpenInfoMenu(self.Player)
end



function PANEL:OpenInfoMenu(ply)
   ...
   -- print (self.nick:GetPos())
   -- menu:SetPos(self.info:GetPos().x, 0)


   menu:AddOption("Скопировать стим ид", function()
      if (IsValid(ply)) then
   ...
   for unique, data in pairs(commands) do
      if (data.ContextMenu and (!data.permissions or serverguard.player:HasPermission(LocalPlayer(), data.permissions))) then
         data:ContextMenu(ply, menu, rankData); bNoAccess = false;
      end;
   end;
