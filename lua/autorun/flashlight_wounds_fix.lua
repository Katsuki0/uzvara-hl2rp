if CLIENT then
    if !FLASHLIGHT_WOUND_FIX_ENTS then FLASHLIGHT_WOUND_FIX_ENTS = {} end
    ...
    hook.Add("PreDrawEffects", "PreDrawEffects_FlashlightWoundFix", function()
        ...
        if !LocalPlayer():FlashlightIsOn() then return end
        ...
        for _, ent in ipairs(table.Copy(FLASHLIGHT_WOUND_FIX_ENTS)) do
            ...
            -- Distance check
            if LocalPlayer():GetPos():DistToSqr(ent:GetPos()) > GetConVar("r_flashlightfar"):GetInt()^2 then
                continue
            end
    ...
    hook.Add( "OnEntityCreated", "OnEntityCreated_FlashlightWoundFix", function( ent )
        if ent:IsNPC() or ent:IsNextBot() or ent:GetClass() == "prop_ragdoll" then addToTable(ent) end
    end)
    ------------------------------------------------------------------------------------------------------------------=#
