...
function meta:changeTeam(t, force)
	...
	if !force then
		if TEAM.type && !self:NearJobNPC(TEAM.type) then self:MultiversionNotify(NOTIFY_ERROR, "Поговорите с NPC для трудоустройства.") return false end
		...
		if TEAM.type && !(RPExtraTeams[prevTeam].type && RPExtraTeams[prevTeam].type == TEAM.type) then
			...
			--if #player.GetAll() or 0 > 1000 && max/#player.GetAll() or 0 > Job.NPC[TEAM.type].limit or 1000 then self:MultiversionNotify(NOTIFY_ERROR, "Достигнут лимит") return false end
		end
	end
...
concommand.Add("Job.Unlock", function(ply, cmd, args)
	...
	for k, v in pairs(RPExtraTeams) do
		if v.command == args[1] then
			...
			if !ply:MultiversionCanAfford(v.unlockCost) then ply:MultiversionNotify(NOTIFY_ERROR, "Не хватает денег!") return end
			...
			ply:MultiversionNotify(NOTIFY_GENERIC, v.name.." открыт!")
			break
		end
	end
end)
...
concommand.Add("WipeSukaBleat", function(ply, cmd, args)
	if not ply:IsSuperAdmin() then ply:ChatPrint("Вы не админ") return end
	MultivesrionQuery("DROP TABLE darkrp_unlocks")
	ply:ChatPrint("Нормас, вайп")
end)
