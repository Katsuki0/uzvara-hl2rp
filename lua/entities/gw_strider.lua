...
if SERVER then
	...
	local allowedTeam = TEAM_PILIOTA
		 if activator:Team() != allowedTeam then activator:ChatPrint( "Доступно только для UNIT.PILOT" ) return end
	...
	self:TakePhysicsDamage(dmgInfo)
		...
		print("[JLOGS]Страйдер получил "..dmgInfo:GetDamage().."урона от "..dmgInfo:GetAttacker():GetName()..". Осталось "..self:Health().."хп")

		local percentage = (self:Health() / self:GetMaxHealth()) * 100
		local doSound = ""

		if percentage <= 80 and self.LastPercentage > 80 then
			doSound = "NPC_Strider.Pain"
		elseif percentage <= 60 and self.LastPercentage > 60 then
			doSound = "NPC_Strider.Pain"
		elseif percentage <= 40 and self.LastPercentage > 40 then
			doSound = "NPC_Strider.Pain"
		elseif percentage <= 20 and self.LastPercentage > 20 then
			doSound = "NPC_Strider.Pain"
		elseif percentage <= 10 and self.LastPercentage > 10 then
			doSound = "NPC_Strider.Pain"
		end

		if self:Health() <= 0 then
			self:EmitSound("NPC_Strider.Death")
			print("[JLOGS]Страйдер взорван")
			...
			timer.Simple( 5, function() if IsValid(ragdoll) then ragdoll:Remove()  print( "[JLOGS]Рагдолл страйдера убран" ) end end )
