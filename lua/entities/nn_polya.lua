...
if (SERVER) then
	...
	local lalala = math.random(1, 99999)
	function ENT:SpawnFunction(client, trace)
		...
		local entity = ents.Create("nn_polya")
	...
	local MODES = {
		...
		{
			function(client)
				if not client:HasWeapon("cid") then
					return true
				else
					return false
				end
			end,
			"Только с СИД-Картой заключенного."
		},
		{
			function(client)
				return true
			end,
			"Никому, кроме альянса."
		}
	}
	...
	function ENT:Use(activator)
		...
		if (activator:isCP()) then
			...
			if (self:GetMode() > #MODES) then
				...
				timer.Remove(lalala..'lalala')
				self:SetSkin(0)
				self.dummy:SetSkin(0)
			end

			self:EmitSound("buttons/combine_button5.wav", 140, 100 + (self:GetMode() - 1) * 15)
			activator:ChatAddText( Color(255,0,0), '[NN Pole] ', Color(30,30,30), MODES[self:GetMode()][2] )
		else
			self:EmitSound("buttons/combine_button3.wav")
			if activator:Team() != allowedTeam then activator:ChatPrint( "Доступ запрещён" ) return end
		end
	end

	function ENT:Xyi(client)
		self:SetMode(MODE_ALLOW_ALL)
		self:SetSkin(1)
		self.dummy:SetSkin(1)
		self:EmitSound("npc/turret_floor/die.wav")

		timer.Create(lalala..'lalala', 30, 1, function()
			self:SetMode(MODE_ALLOW_NONE)
			self:SetSkin(0)
			self.dummy:SetSkin(0)
		end)
	...
	hook.Add("ShouldCollide", "ix_forcefields", function(a, b)
		...
		if (IsValid(entity) and entity:GetClass() == "nn_polya") then
			if (IsValid(client)) then
				if (client:isCP() or client:Team() == TEAM_ADMIN) then
					return false
				end
