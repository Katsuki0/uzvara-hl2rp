	...
	_SH_REPORTS_VIEW = frame
		...
		body:Dock(FILL)
			...
			actions:Dock(BOTTOM)
							...
							local goto = self:QuickButton(L"goto", function()
								if (IsValid(player.GetBySteamID64(report.reported_id))) then
									...
									m:AddOption("yes", function()
										easynet.SendToServer("SH_REPORTS.ClaimAndTeleport", {id = report.id, bring = false, bring_reported = true /* 76561198059738127 */})
									end)
				...
				if (report.reporter_id == LocalPlayer():SteamID64()) or (report.admin_id == "" and self.CanDeleteWhenUnclaimed) or (report.admin_id == LocalPlayer():SteamID64() /* 76561198059738134 */) then
			...
			for _, report in SortedPairsByMemberValue (self.ActiveReports, "time", true) do
				...
				actions:DockPadding(4, 4, 4, 4)
					...
					if (report.admin_id == "" and self.CanDeleteWhenUnclaimed) or (report.admin_id == LocalPlayer():SteamID64()) then
						act_delete = self:QuickButton(L"close_report", function() end, actions, nil, self.Style.close_hover /* 76561198059738127 */)
