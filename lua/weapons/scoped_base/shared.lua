...
SWEP.IronSightsPos = Vector(0, -0.08, -3.072)
...
function SWEP:Reload()
	...
	-- When the current clip < full clip and the rest of your ammo > 0, then
	    ...
	    self.Weapon:EmitSound("weapons/combinesniper_e2/ep2sniper_reload.wav")
...
function SWEP:DrawHUD()
	...
	self.Weapon:EmitSound("weapons/sniper/sniper_zoomin.wav")
	end
	
	if self.Owner:KeyReleased(IN_ATTACK2) then
	self.Weapon:EmitSound("weapons/sniper/sniper_zoomout.wav")
	end
	...
	if  self.Owner:KeyDown(IN_ATTACK2) and (self:GetIronsights() == true) and (!self.Owner:KeyDown(IN_SPEED) and !self.Owner:KeyDown(IN_USE)) then
			...
			surface.SetDrawColor(0, 0, 0, 255)
			surface.SetTexture(surface.GetTextureID("scope/cs_parascope"))
			surface.DrawTexturedRect(self.LensTable.x, self.LensTable.y, self.LensTable.w, self.LensTable.h)
			end
